...
export class Store {
  ...
  getWorkspace(): Workspace | null {
    return (this.db.prepare(`SELECT * FROM workspaces LIMIT 1`).get() as Workspace | undefined) ?? null
  }
  ...
  listEmployees(wsId: string): Employee[] {
    return (this.db.prepare(`SELECT * FROM employees WHERE workspace_id = ?`).all(wsId) as Record<string, unknown>[])
  ...
  listTasks(wsId: string): Task[] {
    return (this.db.prepare(`SELECT * FROM tasks WHERE workspace_id = ? ORDER BY created_at`).all(wsId) as Record<string, unknown>[])
  ...
  subtasksOf(taskId: string): Task[] {
    return (this.db.prepare(`SELECT * FROM tasks WHERE parent_id = ? ORDER BY created_at`).all(taskId) as Record<string, unknown>[])
      .map((r) => this.rowToTask(r))
  }
  ...
  updateTask(id: string, fields: Partial<Pick<Task, 'status' | 'budget_allocated' | 'complexity_note' | 'assignee_id' | 'review_feedback' | 'retries'>>): void {
    ...
    this.db.prepare(`UPDATE tasks SET ${sets.join(', ')} WHERE id = ?`).run(...(vals as never[]))
  }
  ...
  ledgerRecent(wsId: string, limit = 50): unknown[] {
    return this.db.prepare(`SELECT * FROM ledger WHERE workspace_id = ? ORDER BY id DESC LIMIT ?`).all(wsId, limit)
  }
  ...
  inboxResolve(id: string): void {
    this.db.prepare(`UPDATE inbox SET status = 'answered' WHERE id = ?`).run(id)
  }
  ...
  tracesRecent(wsId: string, limit = 30): unknown[] {
    return (this.db.prepare(`SELECT * FROM traces WHERE workspace_id = ? ORDER BY started_at DESC LIMIT ?`).all(wsId, limit) as Record<string, unknown>[])
